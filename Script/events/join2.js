...
.then(response => {
  ...
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    return api.sendMessage("চলে এসেছি আমি ✦───꯭─⃝‌‌𝔹𝔼𝕃𝔸𝕃 𝔹𝕆𝕋 ✡️──✦ তোমাদের মাঝে🤭!", event.threadID, () => api.sendMessage({ body: `${global.config.BOTNAME} CONNECTED«\n\nAssalamualaykum☘️
<------------------------------>  
...
DEVELOPER :┄┉❈✡️⋆⃝চাঁদেড়~পাহাড়✿⃝🪬❈┉┄

🟣Facebook Account Link: 

https://www.facebook.com/mahi.gaming.165

🔵WHATSAPP NUMBER: wa.me/+8801312893012

🟢SUPPORT EMAIL: mzbelalmzbelal@gmail.com`, attachment: fs.createReadStream(gifPath)}, threadID));
  }})
.catch(error => {
    console.error(error);
});
